...
app.post("/make-server-493cc528/auth/reset-all", async (c) => {
  try {
    ...
    return c.json({ success: true, message: `Reset ${users.length} users` });
  } catch (error) {
    console.log(`Reset error: ${error}`);
    return c.json({ error: "Internal server error during reset" }, 500);
  }
});
...
app.post("/make-server-493cc528/auth/signup", async (c) => {
  try {
    ...
    const validRoles = ['STUDENT', 'CAFETERIA_ADMIN', 'SYSTEM_ADMIN'];
    ...
    if (role === 'STUDENT') {
      await kv.set(`budget:${data.user.id}`, {
        userId: data.user.id,
        dailyBudget: 500, // Default 500 KES
        weeklyBudget: 3500, // Default 3500 KES
        currentSpending: 0
      });
    }
...
app.get("/make-server-493cc528/menu/:date", async (c) => {
  try {
    ...
    return c.json({ error: "Internal server error getting menu" }, 500);
  }
});
...
app.post("/make-server-493cc528/menu", async (c) => {
  try {
    ...
    const profile = await kv.get(`user:${user.id}`);
    if (!profile || profile.role !== 'CAFETERIA_ADMIN') {
      return c.json({ error: "Forbidden - Cafeteria Admin access required" }, 403);
    }

    const { date, items } = await c.req.json();
    
    if (!date || !items) {
      return c.json({ error: "Missing date or items" }, 400);
    }
    ...
    return c.json({ error: "Internal server error creating menu" }, 500);
  }
});
...
app.delete("/make-server-493cc528/menu/:date/:itemId", async (c) => {
  try {
    ...
    menu.items = menu.items.filter((item: any) => item.id !== itemId);
    ...
    return c.json({ error: "Internal server error deleting menu item" }, 500);
  }
});
...
app.get("/make-server-493cc528/nutrition", async (c) => {
  try {
    ...
    return c.json({ error: "Internal server error getting nutrition data" }, 500);
  }
});
...
app.post("/make-server-493cc528/nutrition/seed", async (c) => {
  try {
    ...
    if (existing.length > 0) {
      return c.json({ message: "Nutrition database already seeded", count: existing.length });
    }
    ...
    return c.json({ success: true, message: `Seeded ${seededCount} nutrition entries` });
  } catch (error) {
    console.log(`Seed nutrition error: ${error}`);
    return c.json({ error: "Internal server error seeding nutrition data" }, 500);
  }
});
...
app.post("/make-server-493cc528/nutrition", async (c) => {
  try {
    ...
    return c.json({ success: true, nutrition });
  } catch (error) {
    console.log(`Create nutrition error: ${error}`);
    return c.json({ error: "Internal server error creating nutrition data" }, 500);
  }
});
...
app.put("/make-server-493cc528/nutrition/:id", async (c) => {
  try {
    ...
    return c.json({ error: "Internal server error updating nutrition data" }, 500);
  }
});
...
app.delete("/make-server-493cc528/nutrition/:id", async (c) => {
  try {
    ...
    return c.json({ error: "Internal server error deleting nutrition data" }, 500);
  }
});
...
app.get("/make-server-493cc528/budget", async (c) => {
  try {
    ...
    return c.json({ error: "Internal server error getting budget" }, 500);
  }
});
...
app.put("/make-server-493cc528/budget", async (c) => {
  try {
    ...
    const existing = await kv.get(`budget:${user.id}`) || { currentSpending: 0 };
    ...
    return c.json({ error: "Internal server error updating budget" }, 500);
  }
});
...
app.get("/make-server-493cc528/transactions", async (c) => {
  try {
    ...
    const transactions = await kv.getByPrefix(`transaction:${user.id}:`);
    return c.json(transactions.sort((a: any, b: any) => 
...
app.post("/make-server-493cc528/transactions", async (c) => {
  try {
    ...
    return c.json({ error: "Internal server error adding transaction" }, 500);
  }
});
...
app.post("/make-server-493cc528/admin/users", async (c) => {
  try {
    ...
    return c.json({ error: "Internal server error creating user" }, 500);
  }
});
...
app.get("/make-server-493cc528/admin/analytics", async (c) => {
  try {
    ...
    const analytics = {
      ...
      totalCafeteriaAdmins: users.filter((u: any) => u.role === 'CAFETERIA_ADMIN').length,
      totalSystemAdmins: users.filter((u: any) => u.role === 'SYSTEM_ADMIN').length,
      totalTransactions: allTransactions.length,
      totalRevenue: allTransactions.reduce((sum: number, t: any) => sum + (t.amount || 0), 0),
      averageTransactionValue: allTransactions.length > 0 
        ? allTransactions.reduce((sum: number, t: any) => sum + (t.amount || 0), 0) / allTransactions.length 
        : 0
    };
...
app.get("/make-server-493cc528/meal-budgets", async (c) => {
  try {
    ...
    return c.json({ error: "Internal server error getting meal budgets" }, 500);
  }
});
...
app.put("/make-server-493cc528/meal-budgets", async (c) => {
  try {
    ...
    return c.json({ success: true, mealBudgets });
  } catch (error) {
    console.log(`Update meal budgets error: ${error}`);
    return c.json({ error: "Internal server error updating meal budgets" }, 500);
  }
});
...
app.post("/make-server-493cc528/recommendations", async (c) => {
  try {
    ...
    const menu = await kv.get(`menu:${date}`);
    if (!menu || !menu.items) {
      return c.json({ recommendations: [], message: "No menu available for this date" });
    }

    // Filter menu items based on diet preference, budget, and meal category
    let filteredItems = menu.items.filter((item: any) => {
      ...
      if (targetMealType === 'lunch' && !['main course', 'side dish'].includes(itemCategory)) return false;
      if (targetMealType === 'supper' && !['main course', 'side dish'].includes(itemCategory)) return false;
    ...
    return c.json({
      ...
      message: recommendations.length > 0 
        ...
        : "No items match your preferences and budget"
    });
  } catch (error) {
...
app.get("/make-server-493cc528/student/analytics", async (c) => {
  try {
    ...
    const breakfastSpending = transactions
      .filter((t: any) => t.category?.toLowerCase() === 'breakfast')
      .reduce((sum: number, t: any) => sum + (t.amount || 0), 0);
    
    const lunchSpending = transactions
      .filter((t: any) => ['main course', 'side dish'].includes(t.category?.toLowerCase()))
      .reduce((sum: number, t: any) => sum + (t.amount || 0), 0);
    
    const supperSpending = transactions
      .filter((t: any) => t.category?.toLowerCase() === 'supper')
      .reduce((sum: number, t: any) => sum + (t.amount || 0), 0);

    const beverageSpending = transactions
      .filter((t: any) => t.category?.toLowerCase() === 'beverage')
      .reduce((sum: number, t: any) => sum + (t.amount || 0), 0);

    const dessertSpending = transactions
      .filter((t: any) => t.category?.toLowerCase() === 'dessert')
      .reduce((sum: number, t: any) => sum + (t.amount || 0), 0);
